...
jQuery(document).ready(function($){
    ...
    jQuery.fn.essb_get_counters = function(){
        return this.each(function(){
            ...
            function get_alt_url(url, service) {
                "use strict";
                var services={
                    twitter:"https://cdn.api.twitter.com/1/urls/count.json?url=" + url + "&callback=?",
                    pinterest: "https://api.pinterest.com/v1/urls/count.json?callback=?&url=" + url,
                    facebook: "https://graph.facebook.com/fql?q=SELECT%20like_count,%20total_count,%20share_count,%20click_count,%20comment_count%20FROM%20link_stat%20WHERE%20url%20=%20%22"+url+"%22",
                    google: plugin_url+"/public/get-noapi-counts.php?nw=google&url=" + url
                };
            ...
            if ( $twitter.length ) {
                ...
                $('.essb_alternate_url').each(function(idx,el) {
                    "use strict";
                    tdeferreds.push($.getJSON(get_alt_url(el.value,'twitter')));
                });
                $.when.apply($, tdeferreds)
                    .done(function(){
                        var total = 0;
                        $.each(arguments, function() {
                            "use strict";
                            total += this && this.count || this[0] && this[0].count || 0;
                        });
                        counter_display(counter_pos, $twitter, $twitter_inside, total);
                    });
            }
            if ( $linkedin.length ) {
            ...
            if ( $pinterest.length ) {
                ...
                $('.essb_alternate_url').each(function(idx,el) {
                    "use strict";
                    pdeferreds.push($.getJSON(get_alt_url(el.value,'pinterest')));
                });
                $.when.apply($, pdeferreds)
                    .done(function(){
                        var total = 0;
                        $.each(arguments, function() {
                            "use strict";
                        ...
                        counter_display(counter_pos, $pinterest, $pinterest_inside, total);
                    });
            ...
            if ( $google.length ) {
                ...
                $('.essb_alternate_url').each(function(idx,el) {
                    "use strict";
                    gdeferreds.push($.getJSON(get_alt_url(el.value,'google')));
                });
                $.when.apply($, gdeferreds)
                    .done(function(){
                        var total = 0;
                        $.each(arguments, function() {
                            "use strict";
                        ...
                        counter_display(counter_pos, $google, $google_inside, total);
                    });
            ...
            if ( $facebook.length ) {
                ...
                $('.essb_alternate_url').each(function(idx,el) {
                    "use strict";
                    fdeferreds.push($.getJSON(get_alt_url(el.value,'facebook')));
                });
                $.when.apply($, fdeferreds)
                    .done(function(){
                        var total = 0;
                        $.each(arguments, function() {
                            "use strict";
                            total += this && this.count || this[0] && (fb_value == 'true' ? this[0].data[0].total_count : this[0].data[0].share_count) || 0;
                        });
                        counter_display(counter_pos, $facebook, $facebook_inside, total);
                    });
            }
            if ( $del.length ) {
